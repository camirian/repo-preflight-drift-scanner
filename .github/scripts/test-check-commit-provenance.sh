...
commit_as "Repo Owner" "$OWNER" "good: owner commit"
...
echo "$RANDOM" > f.txt; git add f.txt
GIT_AUTHOR_NAME="Repo Owner" GIT_AUTHOR_EMAIL="$OWNER" GIT_COMMITTER_NAME="X" GIT_COMMITTER_EMAIL="$BAD" \
...
GIT_AUTHOR_NAME="Repo Owner" GIT_AUTHOR_EMAIL="$OWNER" GIT_COMMITTER_NAME="GitHub" GIT_COMMITTER_EMAIL="noreply@github.com" \
...
commit_as "Repo Owner" "$OWNER" "feat: add thing" $'feat: add thing\n\nCo-Authored-By: Claude <noreply@anthropic.com>'
...
commit_as "Repo Owner" "$OWNER" "feat: pair session" $'feat: pair session\n\nCo-authored-by: Jane Human <jane@example.com>'
...
commit_as "Repo Owner" "$OWNER" "feat: malformed trailer" $'feat: malformed trailer\n\nCo-authored-by: not-a-valid-line'
...
commit_as "Repo Owner" "$OWNER" "feat: multi" $'feat: multi\n\nCo-authored-by: Claude <noreply@anthropic.com>\nCo-authored-by: Jane Human <jane@example.com>'
...
commit_as "Repo Owner" "$OWNER" "good: unrelated to old bad history"
...
commit_as "Repo Owner" "$OWNER" "feat: on feature branch"
feat_head="$(git rev-parse HEAD)"
git checkout -q -B main_tmp "$head8"
GIT_AUTHOR_NAME="Repo Owner" GIT_AUTHOR_EMAIL="$OWNER" GIT_COMMITTER_NAME="Repo Owner" GIT_COMMITTER_EMAIL="$OWNER" \
