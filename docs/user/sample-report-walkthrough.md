...
The included `examples/sample-repo` is intentionally messy. It exists so users can see what a blocked report looks like without scanning private work.
...
Use `REPO_PREFLIGHT_REPORT.json` for automation examples. The schema artifact path and compatibility rules are documented in [JSON report schema](../report-schema.md).
