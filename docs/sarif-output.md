...
For `dora-ai-readiness`, SARIF is the same view of the profile findings: one DORA-specific result per capability, with `ruleId` values such as `dora_ai_stance_evidence` or `missing_dora_ai_stance`. A SARIF result for DORA evidence is not a certification or approval signal.
...
- For DORA AI readiness scans, treat SARIF as documentation-evidence routing, not proof of DORA maturity, DORA metric performance, governance approval, model safety, or production readiness.
