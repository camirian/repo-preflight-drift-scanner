...
`public_sensitive_term_allowlist` is also additive, but it acts as an explicit waiver list for built-in public-sensitive terms. Use it only for terms that are intentionally public in that repository, such as a profile repository that publishes the owner's public name. It does not disable secret literal checks.
...
```json
{
  ...
  "public_sensitive_term_allowlist": ["public founder name"],
