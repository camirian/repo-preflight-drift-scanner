...
PROFILES = {
    "strict": {
        ...
        "check_secret_filenames": True,
        "check_dora": False,
        "check_fieldheld": False,
    },
    "docs": {
...
DORA_CAPABILITIES = {
    "ai_stance": {
        ...
        "guidance": "Document what AI may do, what requires human review, and what is prohibited.",
    },
    "data_boundary": {
        "label": "data boundary",
        "terms": [
            ...
            "private data",
            "customer artifact",
            "zero-tolerance exposure",
            "no live phi",
        ],
        "guidance": "Name allowed sources, forbidden data, freshness ownership, and retention boundaries.",
    },
    "ai_accessible_context": {
        "label": "AI-accessible context",
        "terms": [
            "ai-accessible",
            "agent context",
            "agents.md",
        ...
        "guidance": "Expose docs, schemas, examples, runbooks, or evidence that agents can safely use.",
    },
    "version_control": {
        "label": "version control and rollback",
        "terms": [
            "version control",
            "rollback",
            "small commits",
        ...
        "guidance": "Describe commit discipline, rollback path, review path, or release artifact handling.",
    },
    "small_batches": {
        "label": "small batches",
        "terms": [
        ...
        "guidance": "Define a small, independently useful next slice with acceptance or kill criteria.",
    },
    "user_focus": {
        "label": "user or user focus",
        "terms": [
            "primary user",
            "target user",
            "user",
        ...
        "guidance": "Name the user or user, their job-to-be-done, and a success or feedback signal.",
    },
    "internal_platform": {
        "label": "internal platform path",
        "terms": [
            "internal platform",
            "golden path",
            "quick start",
        ...
        "guidance": "Provide a repeatable setup, verification, release, or handoff path.",
    },
}

FIELDHELD_CAPABILITIES = {
    "ai_stance": {
        "label": "AI stance",
        "terms": [
            "ai stance",
    ...
    "rollback_path": {
        ...
        "guidance": "Describe commit discipline, rollback path, revert path, or release artifact handling.",
    },
    "small_batch_delivery": {
        "label": "small-batch delivery",
        "terms": [
            "small batch",
            "small-batch",
            "smallest useful",
    ...
    "user_buyer_focus": {
        ...
        "guidance": "Name the user or buyer, their job-to-be-done, and a success or feedback signal.",
    },
    "verification": {
...
CONFIG_LIST_KEYS = {
    "risky_claims",
    "drift_markers",
    "public_sensitive_term_allowlist",
    "secret_filenames",
    "generated_dirs",
    "excluded_dirs",
    "excluded_files",
...
class ScanConfig:
    ...
    public_sensitive_term_allowlist: set[str]
...
def public_sensitive_term_findings(root: Path, path: Path, text: str, config: ScanConfig) -> list[Finding]:
    terms = [term for term in PUBLIC_EXPORT_SENSITIVE_TERMS if term not in config.public_sensitive_term_allowlist]
...
def dora_evidence_findings(root: Path, include_fixtures: bool, config: ScanConfig) -> list[Finding]:
    """Find explicit repo evidence for the seven DORA AI capabilities."""
    evidence: dict[str, Finding] = {}
    for path in sorted(root.rglob("*")):
        if path.is_dir() or path.is_symlink() or should_skip(path, root, include_fixtures, config):
            continue
        ...
        if text is None:
            continue
        lowered_lines = [(number, line, line.lower()) for number, line in enumerate(text.splitlines(), start=1)]
        rel = relative(path, root)
        for capability, spec in DORA_CAPABILITIES.items():
            if capability in evidence:
                continue
            terms = spec["terms"]
            for number, line, lowered in lowered_lines:
                if any(term in lowered for term in terms):
                    evidence[capability] = Finding(
                        level="info",
                        code=f"dora_{capability}_evidence",
                        path=rel,
                        line=number,
                        message=f"DORA evidence found for {spec['label']}.",
                        evidence=line.strip()[:180],
                    )
                    break
    ...
    for capability, spec in DORA_CAPABILITIES.items():
        if capability in evidence:
            findings.append(evidence[capability])
            continue
        findings.append(
            Finding(
                level="blocker",
                code=f"missing_dora_{capability}",
                path=".",
                message=f"Missing DORA AI capability evidence for {spec['label']}: {spec['guidance']}",
            )
        )
    return findings


def fieldheld_evidence_findings(root: Path, include_fixtures: bool, config: ScanConfig) -> list[Finding]:
    """Find explicit repo evidence for the six Fieldheld AI capabilities."""
    evidence: dict[str, Finding] = {}
    for path in sorted(root.rglob("*")):
        if path.is_dir() or path.is_symlink() or should_skip(path, root, include_fixtures, config):
            continue
        ...
        lowered_lines = [(number, line, line.lower()) for number, line in enumerate(text.splitlines(), start=1)]
        rel = relative(path, root)
        for capability, spec in FIELDHELD_CAPABILITIES.items():
            if capability in evidence:
                continue
            terms = spec["terms"]
            for number, line, lowered in lowered_lines:
                if any(term in lowered for term in terms):
                    evidence[capability] = Finding(
                        level="info",
                        code=f"fieldheld_{capability}_evidence",
                        path=rel,
                        line=number,
                        message=f"Fieldheld evidence found for {spec['label']}.",
                        evidence=line.strip()[:180],
                    )
                    break
    ...
    for capability, spec in FIELDHELD_CAPABILITIES.items():
        ...
        findings.append(
            Finding(
                level="blocker",
                code=f"missing_fieldheld_{capability}",
                path=".",
                message=f"Missing Fieldheld AI capability evidence for {spec['label']}: {spec['guidance']}",
            )
        )
    return findings
...
def default_config() -> ScanConfig:
    return ScanConfig(
        ...
        public_sensitive_term_allowlist=set(),
...
def load_config(paths: list[Path]) -> ScanConfig:
    ...
    public_sensitive_term_allowlist = set(config.public_sensitive_term_allowlist)
    ...
    for path in paths:
        ...
        public_sensitive_term_allowlist.update(
            term.lower()
            for term in list_value(payload.get("public_sensitive_term_allowlist"), "public_sensitive_term_allowlist")
        )
    ...
    return ScanConfig(
        ...
        public_sensitive_term_allowlist=public_sensitive_term_allowlist,
...
def scan_repo(
    ...
    if profile_settings["check_dora"]:
        findings.extend(dora_evidence_findings(root, include_fixtures, config))

    if profile_settings.get("check_fieldheld"):
        findings.extend(fieldheld_evidence_findings(root, include_fixtures, config))
    ...
    for path in sorted(root.rglob("*")):
        ...
        if name.lower() in config.secret_filenames or path.suffix.lower() in {".pem", ".key", ".p12"}:
            if profile_settings["check_secret_filenames"]:
                findings.append(
                    Finding(
                        level="blocker",
        ...
        if profile_settings["check_public_export"] and path.name != "repo_preflight.py":
            ...
            findings.extend(public_sensitive_term_findings(root, path, text, config))
...
def new_blocker_count(baseline_diff: dict[str, int | list[dict[str, object]]] | None) -> int:
    if baseline_diff is None:
        return 0
    new_findings = baseline_diff.get("new_findings", [])
    if not isinstance(new_findings, list):
        return 0
    return sum(isinstance(finding, dict) and finding.get("level") == "blocker" for finding in new_findings)
...
def run_scan(args: argparse.Namespace) -> int:
    try:
        if args.fail_on_new_blockers_only and not args.baseline_json:
            raise UserFacingError("--fail-on-new-blockers-only requires --baseline-json")
    ...
    if args.fail_on_new_blockers_only:
        blockers = new_blocker_count(baseline_diff)
        if blockers:
            print("Repo preflight found new blockers.")
            return 1
        if decision(findings) == "BLOCKED":
            print("Repo preflight found blockers already present in baseline.")
            return 0
...
def main() -> int:
    ...
    parser.add_argument("--fail-on-new-blockers-only", action="store_true", help="with --baseline-json, exit non-zero only for blocker findings absent from the baseline")
    ...
    parser.add_argument("--list-profiles", action="store_true", help="print available profiles and exit without scanning")
