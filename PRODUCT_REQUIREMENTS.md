...
Repo Preflight Drift Scanner helps developers and maintainers run a deterministic local preflight before merge, demo, release, public export, or release package delivery.
...
- user of the v0.4 package who needs local CLI setup plus optional GitHub Action setup.
...
- Keep the v0.4 release package useful without requiring external services, accounts, network access, or non-standard-library dependencies.
...
- Package boundary: release package contains QA-useful CLI, Action, examples, configs, and user docs only.
...
- v0.4 packaging keeps seller/internal files outside the QA artifact and preserves user setup paths.
...
- Scanner behavior changes should update `verify_scanner.py`, user docs when user-facing, and release criteria when packaging changes.
